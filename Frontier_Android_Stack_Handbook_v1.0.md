...
KSP (Kotlin Symbol Processing) 2.3.21-2.0.0 替代 kapt，用于 Room 编译期代码生成
...
plugins {
    ...
    // KSP 用于 Room 注解处理器
    id("com.google.devtools.ksp") version "2.3.21-2.0.0" apply false
}
```
...
权限申请 仅 INTERNET + FOREGROUND_SERVICE。不申请存储权限。
...
· 交互逻辑：
  ...
  2. 后台使用 AsyncTask / 协程，将 databases/、files/（含封面图）压缩为 Zip 文件流，写入用户选择的 URI。
...
Fatal 不可恢复的崩溃前夕 常时输出 + 调用 System.exit()
...
2. Kotlin 编译器警告提升：在 app/build.gradle.kts 中增加：
   ...
   tasks.withType<KotlinCompile>().configureEach {
       kotlinOptions {
           allWarningsAsErrors = true  // 日系编程中，警告亦视为错误
           freeCompilerArgs += listOf("-Xexplicit-api-mode=strict") // 强制显式公开 API
       }
   }
   ```
...
系统时间回退至 1970 年 文章列表依然按正常顺序排列 时间戳防御使用 max(timestamp, 0)
...
2. Bitmap内存管理
使用Coil的BitmapPool自动复用；大图使用inSampleSize采样加载；适时调用Bitmap.recycle()。
...
override fun onTrimMemory(level: Int) {
    when (level) {
        TRIM_MEMORY_RUNNING_LOW -> {
            ...
            Coil.get(context).memoryCache?.clear()
        }
